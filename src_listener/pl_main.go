...
func (pl *PluginListener) Create(name string, data string, listenerCustomData []byte) (ax.ExtenderListener, ax.ListenerData, []byte, error) {
	extender, listenerData, customData, listener, err := HandlerCreateListenerDataAndStart(name, data, listenerCustomData)
	if err != nil {
...
func (l *Listener) Edit(config string) (ax.ListenerData, []byte, error) {
	for _, value := range ListenersObject {
		listenerData, customData, ok := HandlerEditListenerData(l.transport.Name, value, config)
		if ok {
...
func (l *Listener) Stop() error {
	...
	for ind, value := range ListenersObject {
		ok, err = HandlerListenerStop(l.transport.Name, value)
		if ok {
			index = ind
			break
		}
...
func (l *Listener) GetProfile() ([]byte, error) {
	data, err := json.Marshal(l.transport.Config)
	if err != nil {
		return nil, err
	}
	return data, nil
